...
const calTBF = (data) => {
  ...
  for (var i = 0; i < data.length; i++) {
    result +=
      (parseInt(data[i].simpleTBF) * weightBMT[0] +
        parseInt(data[i].mediumTBF) * weightBMT[1] +
        parseInt(data[i].complicatedTBF) * weightBMT[2]) *
      coefficientBMT[i];
  }
...
const calTFW = (data) => {
  ...
  for (var i = 0; i < data.length; i++) {
    result +=
      parseFloat(data[i].weightTCF) * parseFloat(data[i].ratingValueTCF);
  }
  return result;
};
